return function(username)
	...
	local text = display.newText("received request from "..username,0,0,200,100,native.systemFontBold, 20)
	...
	for i,v in ipairs{"yes","no"} do 
		...
		btn.touch = function(self,event)
			if event.phase == "ended" then
				if v == "yes" then
					state._consumable.accept_duel = {
						username = username
					}
				else
					state._consumable.decline_duel = {
						username = username
					}
